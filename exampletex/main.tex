...
\fbox{
 ...
 \begin{minipage}{\linewidth}
  ...
  \textnormal{Nowa siła} = 0.25 * \frac{x}{0.25L + x} 
  \end{equation}
 \end{minipage}
}
...
\fbox{
 ...
 \begin{minipage}{\linewidth}
  ...
  \textnormal{factor} &= 1 - \frac{I(n - 1)}{26}
  \end{align}
 \end{minipage}
}
...
\fbox{
 ...
 \begin{minipage}{\linewidth}
  ...
  I(n) &= \left \lfloor{I(n - 1) * \frac{1 - x}{0.77}}\right \rfloor 
  \end{align}
 \end{minipage}
}
...
\fbox{
 ...
 \begin{minipage}{\linewidth}
  \begin{align} 
   \textnormal{Nowa siła} &= \frac{Ax + r(x)}{A + r(x)}  \label{eq:winStrSimulation} \\
  \textnormal{Nowa siła} &= B * \frac{x}{C * L + x} \label{eq:lossStrSimulation} \\
  \label{eq:lossIntervalSimulation}
  I(n) &= \left \lfloor{I(n - 1) * \frac{1 - x}{D}}\right \rfloor \\
   \label{eq:intervalFactorSimulation}
  \textnormal{factor} &= 1 - \frac{I(n - 1)}{E} 
  \end{align}
 \end{minipage}
}
...
Zmienne A,B,C i D przyjmują zestaw wartości ze swoich przedziałów. Wartości, które przyjmują są od siebie równoodległe. T.j każda następna wartość była osiągana poprzez zinkrementowanie poprzedniej o stałą wartość dla danej zmiennej.
